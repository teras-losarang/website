        ...
        <x-col sm="8" lg="8" xl="8" md="8" class="mt-sm-0 mt-3">
            <x-card>
                <x-slot:header class="d-flex justify-content-between align-items-center">
                    <h6 class="card-title mb-0">List Admin</h6>
                    <x-button toggle="modal" target="#addNewUser" size="sm"
                        data-action="{{ route('web.user.admin.store') }}"><x-icon name="fa-solid fa-plus me-2" /> Tambah
                        Admin</x-button>
                </x-slot:header>
                    ...
                    <x-slot:tbody>
                        @foreach ($users as $user)
                            <tr>
                                ...
                                <td nowrap>
                                    @if ($user->id != 1)
                                        <x-button href="{{ route('web.user.admin.update', 1) }}" size="sm"
                                            color="warning me-2" toggle="modal" target="#editNewUser"
                                            data-action="{{ route('web.user.admin.update', $user) }}">
                                            <x-icon name="fa-solid fa-edit me-2" />
                                            Edit</x-button>
                                        <form action="{{ route('web.user.admin.destroy', $user) }}" method="post"
                                            class="d-inline">
                                            @csrf
                                            @method('delete')
...
@push('modal')
    <x-modal id="addNewUser" title="Tambah Admin Baru" :form="true">
    ...
    <x-modal id="editNewUser" title="Edit Admin" :form="true">
        <div id="fetched-data"></div>
        <x-slot:footer>
            <x-button size="sm" color="secondary" dismiss="modal">Batal</x-button>
            <x-button type="submit" size="sm" color="primary">Simpan</x-button>
